...
int main()
{
	...
	string cadena = "";
	...
	//for (size_t i = 0;i<=cadena.size() - 3;++i) {
		...
		if (it != aminoacidos.end())
			if (it->second.second == 1) {
				...
				frecuencia[ami] = frecuencia[ami] + 1;
				++count;
				++cont_gen;
			}
	...
	cout <<"Cantidad de esenciales: "<< count << endl;
	cout <<"Total de aminoacidos: "<< cont_gen << endl;
	cout << endl;
	
	fstream file("frecuencias.txt");
	for (map_si::iterator it = frecuencia.begin(); it != frecuencia.end(); ++it) {
